 ...
 * https://blog.csdn.net/ApeLife/article/details/53574273
 */
    ...
    ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    /* cscfp 指向所有 http 块下的所有 server 块 */
    cscfp = cmcf->servers.elts;
    /* ctx 指向 http 块的 ngx_http_conf_ctx_t 结构 */
    ctx = (ngx_http_conf_ctx_t *) cf->ctx;
    saved = *ctx;
    rv = NGX_CONF_OK;

    /* 遍历 http 块下有所有 server 块 */
    for (s = 0; s < cmcf->servers.nelts; s++) {

        /* merge the server{}s' srv_conf's */

        /* 指向每一个 server 块中的 srv_conf 结构 */
